...
In Section~\ref{sec:models} we have described three main algorithms for identifying similar articles in different languages. These algorithms were $k$-means, LSI and CCA. To test which of these algorithms performs best we performed the following test. Using each algorithm, we analyzed all articles in Event Registry to find the most similar articles in other languages. The values of the cross-lingual article linking features (\texttt{linkCount} and \texttt{avgSimScore}) were therefore recomputed for all annotated learning examples using all three methods. A dataset with only these two features and the label was constructed and used to test how accurate prediction models can be induced using the data. The results of the experiment are shown in Table~\ref{table:linkingEvalAlgos}. The table shows for each of the algorithms the obtained classification accuracy, precision and recall. We can see that the most informative features for cluster linking are obtained using the CCA algorithm, while $k$-means performs the worse. Firstly we verified the AMRR (average of mean reciprocal ranks over all language pairs) performance of approaches by holding out $15000$ aligned test documents and using $300000$ aligned documents as the training set. Figure~\ref{pc:AMRR} shows AMRR score as the function of the number of feature vectors. It is clear that CCA outperforms LSI approach and K-means lags far behind.\todo{the sentences after "Firstly we verified" are not clear - seem out of context}

Furthermore we inspected how number of features influences the accuracy of cluster linking. As we can see from Table~\ref{table:linkingEvalAlgos} choosing number of features vectors larger than $500$ barely effects linking performance. That is probably due to overfitting.

Additionally we also tested how accurately can we link clusters depending on the cluster size. For this experiment we separated the learning examples into two datasets -- one dataset contained clusters where the sum of articles in both clusters is below 20 articles (356 learning examples) while the other dataset contained the rest of the examples (452 examples). 10 fold cross validation was then done separately for each of the two datasets. Results are shown in Table~\ref{table:linkingEvalAlgosLargeSmall}.  As expected, we can see that it is harder to link small clusters than large clusters since we have less articles that provide the necessary information. We can also see that CCA and LSI outperform the $k$-means approach, which is likely due to their high retrieval performance (see Figure~\ref{pic:AMRR}).
...
\item \textbf{Features} - The experiments show that the hub CCA based features present good baseline, which can greatly benefit from additional semantic extraction based features. Even though the CCA based features do not greatly improve the system performance when the semantic features are used, there are two main benefits of the approach. By obtaining a small set of candidate clusters, the cluster linking task can be performed significantly compared to exhaustive evaluation of all cluster pairs. Additionally, for many languages, the named entity and keyword extraction is simply not available which prevents us from using them as learning features.
