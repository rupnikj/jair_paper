...
\item \textbf{probabilistic topic models} - not nice with term weighting, hyperparameters setting, burn-in and other black magic. Representing documents as latent vectors = inference problem.
\item \textbf{matrix factorization} - nonnegative \cite{huertatowards}, svd based \cite{cl_lsi}, cca \cite{Hotelling}, opca \cite{platt2010translingual} - opca makes the assumption that the vocabulary sizes are the same ? that's lame. 
\end{itemize}
