...
\caption{\label{fig:event2} Events are represented by collections of articles about an event, in this case the Malaysian airliner which was shot down over Ukraine. The results shown in the figure can be obtained using the query \url{http://eventregistry.org/event/997350\#?lang=eng\&tab=articles}.}
\end{figure}
...
When building cross-lingual models, the IDF scores were computed with respect to the Wikipedia corpus. In the other part of our system, we computed TFIDF vectors on streams of news documents in multiple languages. There the IDF scores for each language changed dynamically - for each new document we computed the IDF of all news articles within a 10 day window.
...
\begin{comment}
Technical detail. Map everything in the english space. $S_{1, 2} = Q^{en}_1^T Q^{en}_2,$ where  Q^{en}_1 and Q^{en}_2 are orthogonal matrices. All non zero singular values are thus automatically equal to one.
\end{comment}
...
Sum of Squared Correlations (SSCOR) between several language pairs, which was introduced in \cite{Kettenring}. The SSCOR problem is difficult to solve in our setting (hundreds of thousands of features, hundreds of thousands of examples). To tackle this, we propose a method which consists of two ingredients.
 ...
 makes the eigenvector problem computationally and numerically tractable. \todo{we need to set this up better. Revised by Jan. Check if OK. Andrej checked}
...
\todo{Missing matrix S of singular values!!!}
\end{figure}
...
In the case of Wikipedia we observed that even though the training resources are scarce for certain language pairs, there often exists indirect training data that we can use as a proxy -- hub language.\todo{Check this, I prettified it.}
...
\subsection{Experiments With Missing Alignment Data}\label{experiments:hubcca}
 ...
 \todo{tfidf over what corpus  - is this static? Andrej: It is always static in hub based approach, i think based on en - es alignment in this case!}
 ...
 Although the English language is well aligned with other languages,\todo{well-aligned with what} number of articles highly correlates with the
