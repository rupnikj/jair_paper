...
The paper is organized as follows: we first provide an overview of the system as a whole in Section~\ref{sec:pipeline}, which includes a subsection that summarizes the main system requirements. We then present related work in Section~\ref{sec:related}. The related work covers work on cross-lingual document similarity as well as work on cross-lingual cluster linking. In Section~\ref{sec:crosslingual}, we introduce the problem of cross-lingual document similarity computation, which is followed by an overview  and description of the main cross-lingual similarity approaches we consider in detail, including a new approach based on hub languages.
...
In this section, we will describe several approaches to the problem of computing multilingual similarities introduced in Section~\ref{sec:tfidf}. We present four approaches:
...
We will now describe four models to cross-lingual similarity computation in the next sections.
...
In the next section we will describe our second approach.
...
In the next section we will introduce our third approach.
...
Note that the method in its original form is only applicable to two languages where an aligned set of observations is available. The next section will describe a scalable an extension of CCA to more than two languages.
