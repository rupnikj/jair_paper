...
method to enables cross-lingual document similarity based on Wikipedia, which
 enables us to compute the similarity of any two articles regardless of language. Using this similarity, we then propose an approach to find and match clusters across languages which represent the same event. We provide an extensive evaluation of the system as a whole, as well as an evaluation of the quality of the similarity measure and linking algorithm.
 \begin{comment}
Main points:
\begin{itemize}
...
As input we consider a high volume of articles in different languages. By using a language detector the stream is split into separate monolingual streams. 
Within each monolingual stream, an online clustering approach is employed, where tracked clusters correspond to our definition of events - this is based on the Event Registry system \shortcite{Leban2014W,Leban2014I}. Our main goal is to connect such clusters (representations of events) across languages, that is detect that a set of documents in language 1 reports on the same event as a set of documents in language 2.

Our approach to combines two ingredients: a cross-lingual document similarity measure, which can be interpreted as a language independent topic model, and
semantic annotation of documents, which enables an alternative way to comparing documents by comparing the lists of detected semantic entities.
...
The paper is organized as follows: we first provide an overview of the system as a whole (Section ~\ref{sec:pipeline}), followed by details on each of the two steps described above. We first introduce several approaches to measures of cross-lingual similarity (Section~\ref{sec:crosslingual}) and then the algorithms for matching clusters iin different languages(Section~\ref{sec:linking}).  To improve readability, we describe relevant related work for each module in its corresponding Section.
