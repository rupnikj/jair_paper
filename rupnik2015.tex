...
Since articles in a cluster are in a single language, we also want to identify any other existing clusters that are reporting about the same event in another language and join these clusters into the same event. This task is performed using a classification approach that is the second major contribution of this paper. It is described in details in Section~\ref{sec:linking}.
...
We will describe the main dataset for building cross-lingual models which is based on Wikipedia. After that we will present two sets of experiments. The first set of experiments
...
that we presented on the task of mate retrieval and the task of event linking. We will also examine how different choices of features impact the event linking performance.
...
\subsection{Experiments With Missing Alignment Data}\label{experiments:hubcca}
 ...
 Although the English language is well aligned, number of articles highly correlates with the
